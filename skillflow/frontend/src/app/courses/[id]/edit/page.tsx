...
export default function EditCoursePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      await apiFetch(`/courses/${id}`, { method: 'PATCH', body: JSON.stringify({ ...form, sections }) })
    ...
    <div style={{ background: '#F5F7F8', minHeight: '100vh', fontFamily: 'var(--font-body)' }}>
      <nav style={{ background: '#fff', borderBottom: '1px solid #E0E0E0', height: 56, display: 'flex', alignItems: 'center', position: 'sticky', top: 0, zIndex: 50 }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <Link href={`/courses/${id}`} style={{ fontSize: 14, color: '#5C5C5C', fontWeight: 500, textDecoration: 'none' }}>← Back to course</Link>
        </div>
      </nav>

      <div className="container" style={{ maxWidth: 760, paddingTop: 40, paddingBottom: 80 }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: '#1F1F1F', letterSpacing: '-0.02em', marginBottom: 4 }}>Edit Course</h1>
        <p style={{ fontSize: 14, color: '#5C5C5C', marginBottom: 36 }}>Update course details, sections, and lessons.</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: '#fff', border: '1px solid #E0E0E0', borderRadius: 4, padding: 28, marginBottom: 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                rows={3} style={{ ...INPUT, resize: 'vertical' }} placeholder="What will students learn?" />
            </div>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: 14, color: '#5C5C5C' }}>Published (visible to all learners)</span>
            </label>
          </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #E0E0E0', borderRadius: 4, padding: 28, marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: '#1F1F1F' }}>Sections & Lessons</h2>
              <button type="button" onClick={addSection} style={{ padding: '7px 16px', background: '#EFF6FF', color: '#0056D2', border: '1px solid #BFDBFE', borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                + Add Section
              </button>
            </div>
                  ...
                  {sec.lessons.map((lesson, li) => (
                    <div key={li} style={{ display: 'grid', gridTemplateColumns: '1fr 110px auto', gap: 8, marginBottom: 10, alignItems: 'start' }}>
                      ...
                      {sec.lessons.length > 1 && (
                        <button type="button" onClick={() => removeLesson(si, li)} style={{ background: 'none', border: 'none', color: '#DC2626', cursor: 'pointer', fontSize: 18, lineHeight: 1, paddingTop: 8 }}>×</button>
                      )}
                    </div>
                  ))}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Link href={`/courses/${id}`} style={{ padding: '12px 20px', border: '1px solid #E0E0E0', borderRadius: 4, fontSize: 14, fontWeight: 500, color: '#5C5C5C', background: '#fff', textDecoration: 'none' }}>
              Cancel
            </Link>
          </div>
        </form>
