      ...
      <nav style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)', height: 56, display: 'flex', alignItems: 'center', position: 'sticky', top: 0, zIndex: 50 }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <ThemeToggle />
            <Link href={`/courses/${id}`} style={{ fontSize: 14, color: 'var(--text-secondary)', fontWeight: 500, textDecoration: 'none' }}>← Back to course</Link>
          </div>
        </div>
      </nav>

      <div className="container" style={{ maxWidth: 760, paddingTop: 40, paddingBottom: 80 }}>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 4 }}>Edit Course</h1>
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 36 }}>Update course details, sections, and lessons.</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 4, padding: 28, marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Sections & Lessons</h2>
              <button type="button" onClick={addSection} style={{ padding: '7px 16px', background: 'var(--accent-dim)', color: 'var(--accent)', border: '1px solid var(--border)', borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                + Add Section
              </button>
            </div>
                  ...
                  {sec.lessons.map((lesson, li) => (
                    <div key={li} style={{ display: 'grid', gridTemplateColumns: '1fr 110px auto', gap: 8, marginBottom: 10, alignItems: 'start' }}>
                      ...
                      {sec.lessons.length > 1 && (
                        <button type="button" onClick={() => removeLesson(si, li)} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 18, lineHeight: 1, paddingTop: 8 }}>×</button>
                      )}
                    </div>
                  ))}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Link href={`/courses/${id}`} style={{ padding: '12px 20px', border: '1px solid var(--border)', borderRadius: 4, fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)', background: 'var(--bg-alt)', textDecoration: 'none' }}>
              Cancel
            </Link>
          </div>
        </form>
